...
class MusicCtrl:
    ...
    def stop(self):
        ...
        self.model.sonification_view.set_status_text("semaphore: {}/{}, {}/{}, {}".format(self.emptySemaphore._value,
                                                   self.emptySemaphore._initial_value,
                                                   self.fullSemaphore._value,
                                                   self.fullSemaphore._initial_value,
                                                   self.queueSemaphore._value))
    ...
    def import_all_tracks(self, path): #TODO worki8ng if there are already tracks?
        with open(path, 'rb') as f:
            m = pickle.load(f)
            sview = self.model.sonification_view
            self.purge_tracks()
            ...
            self.model.sonification_view = sview
    ...
    def push_data_to_table(self, datas):
        for data in datas.itertuples():
            if not ((data.internal_id == self.data.get_first().internal_id).any() or
                ...
                self.model.sonification_view.tableView.data_model.loadRow(data)
