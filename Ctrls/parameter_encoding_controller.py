        ...
        if self.model.encoded_var == "value":
            if value.isnumeric():
                try:
                    ...
                    logging.log(logging.WARNING, "Value sent to the pec is numeric but is not a valid note: {}.".format(value))
                    return
            else:
                try:
                    v = Models.note_model.note_to_int(str(value).upper(), int(self.model.octave))
                    ...
                    logging.log(logging.WARNING, "Value sent to the pec is not numeric but is not a valid note: {}.".format(value))

                    return
        elif value.isnumeric():
            self.model.handpickEncoding[variable] = int(value) if self.model.maxValue > int(value) > 0 else self.model.defaultValue
