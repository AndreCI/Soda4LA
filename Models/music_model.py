...
class Music:
    ...
    def generate(cls):
        """
        Must be called as a thread
        Iterate over the data, generate all the notes for all the tracks, so that they can be played.
        Notes are generated by batch every BUFFER_TIME_LENGTH seconds.
        """
        cls.notes = [] #Reset current notes
        #TODO generate and stores notes batch by batch, like this?
        while cls.data.get_next().empty is False:
            for t in cls.tracks:
                cls.notes.extend(t.generate_notes(cls.data.get_next()))
                #No need to reorganise cls.notes by time factor as pyfluidsynth will do it for us
